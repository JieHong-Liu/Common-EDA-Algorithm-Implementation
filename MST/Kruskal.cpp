...
public:
	Point() { this->id = 0; this->x = 0; this->y = 0; }
...
class Edge {
public:
	Edge() { this->p1 = 0; this->p2 = 0; this->weight = 0; }
	Edge(int p1, int p2, int weight) { this->p1 = p1; this->p2 = p2; this->weight = weight; }
	int p1; // id of p1;
	int p2; // id of p2;
...
void printGraph(vector<pair<int, int> > adj[], int V,vector<Point>& pointList)
{
	...
	for (int u = 0; u < V; u++)
	{
		cout << "Node " << u <<"("<< pointList[u].x <<" , " << pointList[u].y << ")" << " makes an edge with \n";
		for (auto it = adj[u].begin(); it != adj[u].end(); it++)
		{
			v = it->first;
			w = it->second;
			cout << "\tNode " << v << "(" << pointList[v].x << " , " << pointList[v].y << ")"  " with edge weight ="
				<< w << "\n";
		}
		cout << "\n";
	}
...
{	
	...
	pointList.push_back(Point(0, 1, 24)); // id,x,y
	pointList.push_back(Point(1, 8, 16));
	pointList.push_back(Point(2, 32, 4));
	pointList.push_back(Point(3, 12, 24));
